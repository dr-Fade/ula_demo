...
# ╔═╡ d7db37d4-5e11-4b45-a7a1-baaf4161e8d9
using Statistics

# ╔═╡ 768cfc1f-6c7c-4a26-8157-76ec5e1a0fbe
using Random

# ╔═╡ 3fa7116a-71c8-419f-b99f-70583cd9ab87
using FFTW

# ╔═╡ f27f6bde-1586-11f1-35fd-b3518800e61e
md"# ULA array demo"

# ╔═╡ aa969531-3140-47ea-97c4-2e68702cded5
md"""
...
# ╔═╡ 9b8e2964-39e8-41bd-b8cd-25ac541fd44c
md"""
...
$$\theta = \text{argmax} \left( \dfrac{1}{s^H V_n V^H_n s} \right),$$
...
md"SNR (dB): $(@bind snr_db Slider(0:100, show_value=true))"

# ╔═╡ 748202d5-94a3-4856-ba1d-95e020db5aae
md"DOA: $(@bind doa Slider(-90:90, default=30, show_value=true))"

# ╔═╡ 41146067-9289-41f6-8bcb-2b5c342ab024
md"Frequency (MHz): $(@bind f Slider(0:10, default=0, show_value=true))"

# ╔═╡ 0d35913f-e252-408a-9569-ce60a256856a
function plot_signal(t, signal, steered_signal, weighted_signal)
    plt = plot(
        t, real(signal), 
    ...
    for i in 1:size(steered_signal, 1)
        plot!(plt, t, real(steered_signal[i, :]), 
              # label="Received Signal $i", 
              linestyle=:dash, 
              linewidth=1.5)
    end
    
    plot!(plt, t, real(weighted_signal), 
          label="Weighted Signal", 
          linewidth=2)
    
    return plt
end

# ╔═╡ 7f2fcd79-cb19-4ef3-94a2-eb3ec8ad0e1e
function plot_θ_scan(Θ, p, estimate_threshold, scan_source)
    # convert to radar convention (0° = North, clockwise)
    theta_plot = Θ
    ...
    for i in 2:length(p)-1
        if p[i-1] < p[i] && p[i] > p[i+1] && p[i] > estimate_threshold
            angle_deg = Θ[i] * 180 / π
            theta_peak = Θ[i]
            r_peak = p[i]
            plot!(
				...
				[0, r_peak],
				linewidth=1.5,
				label=string(round(angle_deg, digits=1)) * "°"
			)
        end
    end
...
# ╔═╡ 43cb8365-b177-40f0-945f-bcbe441860cf
md"# ULA implementation"

# ╔═╡ 6a42eb50-8c6b-4b14-a9cc-07a8aecf6035
struct ULA
    N::Int
    d::Float64
	function ULA(N::Int, d::Float64=0.5)
    	new(N, d)
	end
end

# ╔═╡ cd23c0f1-1f69-487b-a690-fd5d21e14803
ula = ULA(8, 0.5)

# ╔═╡ 91a809d4-e010-4504-b7cd-757edb2c7e1d
function _get_steering(ula::ULA, θ::Real)
    exp.(2im * π * ula.d * (0:(ula.N-1)) * sin(θ))
end

# ╔═╡ ef07e952-9c1d-4dbf-bc6e-357c3378409d
function steer_signal(ula::ULA, signal::AbstractVector, θ::Real)
    x = signal'
    steering = _get_steering(ula, θ)
    steering * x
end

# ╔═╡ d7e796c9-bb85-48d1-a17d-abc5a00a67b4
function delay_and_sum(ula::ULA, steered_signal::AbstractMatrix, θ::Real)
    if size(steered_signal, 1) != ula.N
        throw(ArgumentError("invalid signal dimensions $(size(steered_signal)), expected ($(ula.N) x n)"))
    end

    result = _get_steering(ula, θ)' * steered_signal
    vec(result)
end

# ╔═╡ 79ad4310-c3b5-4e29-99c3-b1c89a179559
function music(ula::ULA, steered_signal::AbstractMatrix, n_signals::Int, n::Int=360)
    R = cov(steered_signal, dims=2)
    eig = eigen(R)
    eig_val_order = sortperm(abs.(eig.values))
    eigvecs = eig.vectors[:, eig_val_order]
    K = ula.N - n_signals
    V = eigvecs[:, 1:K]
    ...
    results = [
		begin
	        steering = _get_steering(ula, θ)
	        proj = V * V'
	        metric = 1 / (steering' * proj * steering)
	        metric = abs(metric)
	        metric = 10 * log10(metric)
    	end 
    	for θ in Θ
	]
    
    results .-= minimum(results)
    results ./= maximum(results)

	(Θ, results)
end

# ╔═╡ f27b7ca4-2731-4298-821f-adc7462ee1c9
md"# Signal utils"

# ╔═╡ 5ab1746d-8d1d-49eb-8cbb-aed0a9501c5f
function power(signal::AbstractVector) :: Float64
    n = length(signal)
    return sum(abs2, signal) / n
end

# ╔═╡ 4d921e1f-24c1-4f92-a6a4-74f9005ed2aa
function rms(signal::AbstractVector) :: Float64
    return sqrt(power(signal))
end

# ╔═╡ 565e1e74-25bd-44d6-be5f-bb4a2359c932
function get_θ_scan(ula::ULA, steered_signal::AbstractMatrix, n::Int=360)
    if size(steered_signal, 1) != ula.N
        throw(ArgumentError("invalid signal dimensions $(size(steered_signal)), expected ($(ula.N) x n)"))
    end
	...
	Θ = range(-π, π, length=n)
    p = [
		rms(delay_and_sum(ula, steered_signal, θ))
		for θ in Θ
	]
...
# ╔═╡ aa804598-b894-4964-a17e-1d0b7d4050ad
function add_noise(signal::AbstractVector, snr::Float64)
    signal_power = sum(abs2, fft(signal))
    noise_power = signal_power / snr

    noise = randn(length(signal))
    noise_fft = fft(noise)
    current_noise_power = sum(abs2, noise_fft)

    scaling_factor = sqrt(noise_power) / sqrt(current_noise_power)
...
begin
    ...
    # beamforming
    weighted_signal = delay_and_sum(ula, steered_signal, θ)
    
    # generate plots
    signals_plt = plot_signal(t, signal, steered_signal, weighted_signal)
    
    # delay-and-sum scan
    Θ_das, p_das = get_θ_scan(ula, steered_signal)
    threshold_das = 0.9 * power(signal) * ula.N
    delay_and_sum_scan = plot_θ_scan(Θ_das, p_das, threshold_das, "Delay-and-Sum")
    
    # MUSIC scan
    Θ_music, p_music = music(ula, steered_signal, 2)
    threshold_music = 0.5
    music_scan = plot_θ_scan(Θ_music, p_music, threshold_music, "MUSIC")
...
manifest_format = "2.0"
project_hash = "0c2dbfb6449cc7ec7953154efd2dec8f507f3061"
